...
const PRODUCTS = [
  {
    sku: 'ISB6117',
    name: 'San Benedetto 0,5 l – perlivá',
    brand: 'San Benedetto',
    collection: 'Premium',
    volume: '0,5 l',
    weight: '0,5 kg',
    waterType: 'Perlivá',
    categories: ['Premium', 'San Benedetto', 'Minerální voda', 'Zlatá řada', 'Objem 500 ml'],
    ...
    skins: [
      { src: 'assets/bottle_500ml_your_logo_sanbenedetto_2.png', swatch: '#d9822f', label: { x: 0.390, y: 0.566, w: 0.217, h: 0.185 } }
    ]
  },
  {
    sku: 'ISB6118',
    name: 'San Benedetto 0,5 l – neperlivá',
    brand: 'San Benedetto',
    collection: 'Premium',
    volume: '0,5 l',
    weight: '0,5 kg',
    ...
    skins: [
      { src: 'assets/bottle_500ml_your_logo_sanbenedetto.png', swatch: '#2f7d78', label: { x: 0.399, y: 0.600, w: 0.203, h: 0.147 } }
    ]
  },
  {
    sku: 'ISB6116',
    name: 'San Benedetto 0,33 l – neperlivá',
    brand: 'San Benedetto',
    collection: 'Premium',
    volume: '0,33 l',
    weight: '0,33 kg',
    waterType: 'Neperlivá',
    categories: ['Premium', 'San Benedetto', 'Minerální voda', 'Zlatá řada', 'Objem 330 ml'],
    tags: ['San Benedetto', 'minerální voda', 'PET', 'neperlivá'],
    ...
    skins: [
      { src: 'assets/bottle_330ml_your_logo_2.png', swatch: '#1f5f63', label: { x: 0.361, y: 0.550, w: 0.277, h: 0.178 } }
    ]
  },
  {
    sku: 'ISB6115',
    name: 'San Benedetto 0,33 l – perlivá',
    brand: 'San Benedetto',
    collection: 'Premium',
    volume: '0,33 l',
    weight: '0,33 kg',
    ...
    skins: [
      { src: 'assets/bottle_330ml_your_logo.png', swatch: '#2f7d78', label: { x: 0.361, y: 0.550, w: 0.277, h: 0.178 } }
    ]
  },
  {
    sku: 'ISB2112',
    name: '0,5 l – jemně perlivá',
    brand: 'Core',
    collection: 'Core',
    volume: '0,5 l',
    weight: '0,5 kg',
    waterType: 'Jemně perlivá',
    categories: ['Core', 'Pitná voda', 'Objem 500 ml'],
    ...
    skins: [
      { src: 'assets/bottle_500ml_your_logo_2.png', swatch: '#d9822f', label: { x: 0.390, y: 0.566, w: 0.217, h: 0.185 } }
    ]
  },
  {
    sku: 'ISB2111',
    name: '0,5 l – neperlivá',
    brand: 'Core',
    collection: 'Core',
    volume: '0,5 l',
    weight: '0,5 kg',
    ...
    skins: [
      { src: 'assets/bottle_500ml_your_logo.png', swatch: '#2f7d78', label: { x: 0.399, y: 0.600, w: 0.203, h: 0.147 } }
    ]
  }
];
...
function drawBottle(canvas, opts) {
  ...
  if (opts.image) {
    ...
    ctx.save();
    ctx.translate(rect.x + rect.w / 2 + t.offsetX, rect.y + rect.h / 2 + t.offsetY);
...
function renderCategoryFilters() {
  ...
  const all = new Set();
  PRODUCTS.forEach(p => p.categories.forEach(c => all.add(c)));
  const cats = ['Vše', ...all];
  wrap.innerHTML = '';
  cats.forEach(cat => {
...
function renderProductGrid() {
  ...
  list.forEach(product => {
    ...
    badges.innerHTML = `<span class="badge">Kolekce ${escapeHtml(product.collection)}</span><span class="badge badge-outline">${escapeHtml(product.volume)}</span><span class="badge badge-outline">${escapeHtml(product.waterType)}</span>`;
    ...
    detailsWrap.className = 'product-details-wrap';

    const detailsToggle = document.createElement('button');
    detailsToggle.type = 'button';
    detailsToggle.className = 'details-toggle';
    detailsToggle.setAttribute('aria-expanded', 'false');
    detailsToggle.innerHTML = '<span>Zjistit více</span><span class="details-chevron">⌄</span>';

    const details = document.createElement('div');
    details.className = 'product-details';
    details.hidden = true;
    details.innerHTML = buildDetailsHtml(product);
    ...
    detailsToggle.addEventListener('click', () => {
      ...
      detailsToggle.querySelector('span').textContent = isOpen ? 'Zjistit více' : 'Skrýt informace';
    });

    detailsWrap.append(detailsToggle, details);
    ...
    btn.addEventListener('click', () => openCustomizer(product));
    actions.appendChild(btn);

    card.append(canvas, badges, h3, tagline, priceRow, moq, detailsWrap, actions);
...
document.getElementById('rotate-range').addEventListener('input', e => {
  state.transform.rotate = Number(e.target.value);
  redrawDesignCanvas();
});
...
const SHIPPING_RATE_NORMAL = 2.0;   // Kč / km
const SHIPPING_RATE_LARGE = 4.5;  // Kč / km, když objednávka přesáhne kapacitu jedné várky
...
function renderCheckoutTotals() {
  ...
  wrap.innerHTML = `
    ...
    <div class="row"><span>Doprava (${escapeHtml(state.shipping.label)})</span><span>${shippingCost > 0 ? formatKc(shippingCost) : 'zdarma'}</span></div>
...
document.getElementById('calc-shipping-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (isPragueAddress(geo)) {
      ...
      resultEl.textContent = 'Doprava byla nastavena.';
      renderCheckoutTotals();
      return;
    }
    ...
    state.shipping = {
      method: 'outside',
      cost,
      label: `Doručení na adresu (~${distOneWay.toFixed(0)} km)`
    };
    resultEl.textContent = 'Doprava byla nastavena.';
    renderCheckoutTotals();
  } catch (err) {
...
document.getElementById('checkout-form').addEventListener('submit', async e => {
  ...
  const body = [
    ...
    ...lines,
    ``,
    `Doprava: ${state.shipping.label}${shippingCost > 0 ? ' — ' + formatKc(shippingCost) : ''}`,
    ...
    ``,
    `(Pozn.: soubory s motivy si prosím připravte k samostatnému odeslání, tento formulář posílá jen textový souhrn. Cena dopravy na adresu je orientační, počítaná podle skutečné silniční trasy. Referenční kód není oficiální číslo objednávky.)`
  ].join('\n');
