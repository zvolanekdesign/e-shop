...
const PRODUCTS = [
  {
    ...
    weight: '0,5 kg',
    waterType: 'Perlivá',
    categories: ['Minerální voda', 'San Benedetto', 'Zlatá řada', 'Objem 500 ml'],
  ...
  },
  {
    ...
    skins: [
      { src: 'assets/bottle_500ml_your_logo_sanbenedetto_2.png', swatch: '#2f7d78', label: { x: 0.399, y: 0.600, w: 0.203, h: 0.147 } }
    ]
  },
  {
    sku: 'ISB6116',
    ...
    weight: '0,33 kg',
    waterType: 'Neperlivá',
    categories: ['Minerální voda', 'San Benedetto', 'Zlatá řada', 'Objem 330 ml'],
    tags: ['San Benedetto', 'minerální voda', 'PET', 'neperlivá'],
    ...
    skins: [
      { src: 'assets/bottle_500ml_your_logo_sanbenedetto.png', swatch: '#1f5f63', label: { x: 0.361, y: 0.550, w: 0.277, h: 0.178 } }
    ]
  },
  {
    sku: 'ISB6115',
    ...
    skins: [
      { src: 'assets/bottle_500ml_your_logo.png', swatch: '#2f7d78', label: { x: 0.361, y: 0.550, w: 0.277, h: 0.178 } }
    ]
  },
  {
    sku: 'ISB2112',
    name: 'Core 0,5 l PET – jemně perlivá',
    brand: 'Core',
    volume: '0,5 l',
    weight: '0,5 kg',
    waterType: 'Jemně perlivá',
    categories: ['Pitná voda', 'Core', 'Objem 500 ml'],
    ...
    skins: [
      { src: 'assets/bottle_330ml_your_logo.png', swatch: '#d9822f', label: { x: 0.390, y: 0.566, w: 0.217, h: 0.185 } }
    ]
  },
  {
    sku: 'ISB2111',
    name: 'Core 0,5 l PET – neperlivá',
    brand: 'Core',
    volume: '0,5 l',
    weight: '0,5 kg',
    waterType: 'Neperlivá',
    ...
    skins: [
      { src: 'assets/bottle_330ml_your_logo_2.png', swatch: '#2f7d78', label: { x: 0.399, y: 0.600, w: 0.203, h: 0.147 } }
    ]
  }
];
...
function renderProductGrid() {
  ...
  list.forEach(product => {
    ...
    badges.innerHTML = `<span class="badge">${escapeHtml(product.brand)}</span><span class="badge badge-outline">${escapeHtml(product.volume)}</span><span class="badge badge-outline">${escapeHtml(product.waterType)}</span>`;
    ...
    detailsToggle.textContent = 'Zjistit více';
    ...
    detailsToggle.addEventListener('click', () => {
      ...
      detailsToggle.textContent = details.hidden ? 'Zjistit více' : 'Skrýt informace';
    });
    ...
    card.append(canvas, badges, h3, tagline, priceRow, moq, detailsToggle, details, actions);
...
const SHIPPING_RATE_NORMAL = 5;   // Kč / km
const SHIPPING_RATE_LARGE = 7.5;  // Kč / km, když objednávka přesáhne kapacitu jedné várky
...
document.getElementById('calc-shipping-btn').addEventListener('click', async () => {
  ...
  resultEl.textContent = 'Hledám adresu…';
  try {
    ...
    resultEl.textContent = 'Počítám trasu…';
    ...
    resultEl.textContent =
      `Trasa po silnici ${distOneWay.toFixed(1)} km, tam i zpět ${distRoundTrip.toFixed(1)} km × ${formatKc(rate)}/km ≈ ${formatKc(cost)}.`;
    renderCheckoutTotals();
  } catch (err) {
