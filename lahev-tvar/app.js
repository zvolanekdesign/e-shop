...
const PRODUCTS = [
  {
    ...
    skins: [
      { src: 'assets/bottle_500ml_your_logo_sanbenedetto_2.png', swatch: '#188fa7', label: { x: 0.3907, y: 0.5861, w: 0.2176, h: 0.1668 } }
    ]
  },
  {
    sku: 'ISB6118',
    ...
    skins: [
      { src: 'assets/bottle_500ml_your_logo_sanbenedetto.png', swatch: '#2b8da0', label: { x: 0.3985, y: 0.5909, w: 0.2045, h: 0.1566 } }
    ]
  },
  {
    sku: 'ISB6116',
    ...
    skins: [
      { src: 'assets/bottle_330ml_your_logo_2.png', swatch: '#2b8da0', label: { x: 0.3617, y: 0.5412, w: 0.2769, h: 0.195 } }
    ]
  },
  {
    sku: 'ISB6115',
    ...
    skins: [
      { src: 'assets/bottle_330ml_your_logo.png', swatch: '#188fa7', label: { x: 0.3617, y: 0.5412, w: 0.2769, h: 0.195 } }
    ]
  },
  {
    sku: 'ISB2112',
    ...
    skins: [
      { src: 'assets/bottle_500ml_your_logo_2.png', swatch: '#188fa7', label: { x: 0.4067, y: 0.5884, w: 0.1874, h: 0.1547 } }
    ]
  },
  {
    sku: 'ISB2111',
    ...
    skins: [
      { src: 'assets/bottle_500ml_your_logo.png', swatch: '#2b8da0', label: { x: 0.4067, y: 0.5884, w: 0.1874, h: 0.1547 } }
    ]
  }
];
...
function renderProductGrid() {
  ...
  list.forEach(product => {
    ...
    btn.addEventListener('click', () => { location.href = `product.html?sku=${encodeURIComponent(product.sku)}`; });
...
/** Vykreslí plný popis produktu (složení, ceník, značka…) na product.html. */
function renderProductInfo(product) {
  const wrap = document.getElementById('product-info');
  if (!wrap) return;
  wrap.innerHTML = `<h2 class="product-info-title">${escapeHtml(product.name)}</h2><div class="product-details product-details-full">${buildDetailsHtml(product)}</div>`;
}

/** Vykreslí mřížku ostatních produktů (bez aktuálně zobrazeného) pod detailem. */
function renderRelatedProducts(currentProduct) {
  ...
  others.forEach(product => {
    ...
    card.append(canvas, badges, h3, tagline, priceRow, actions);
    grid.appendChild(card);
  });
}
...
document.getElementById('add-to-cart').addEventListener('click', () => {
  ...
  if (addBtn) { addBtn.textContent = 'Přidáno do košíku ✓'; setTimeout(() => { addBtn.textContent = 'Přidat další kus do košíku'; }, 1800); }
});
...
function renderCart() {
  ...
  if (!itemsWrap) return; // na této stránce není celý košík, jen odznak v hlavičce
...
loadCart();
renderCart();
if (document.getElementById('category-filters')) renderCategoryFilters();

const needsSkins = document.getElementById('product-grid') || document.getElementById('hero-canvas') || IS_PRODUCT_PAGE;
if (needsSkins) {
  preloadSkins().then(() => {
    if (document.getElementById('product-grid')) renderProductGrid();
    if (document.getElementById('hero-canvas')) drawHero();
    if (IS_PRODUCT_PAGE) {
